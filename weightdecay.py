...
lbvecs = np.zeros((48000,10))
for i in range(48000):
    l = trlbls[i]
    lbvecs[i][l]=1
del trlbls
# -- NORMALIZATION OF INPUT VECTORS --
trimgs = np.resize(trimgs,(48000,784))
trimgs1 = []
for v in trimgs:
...
tsimgs = np.resize(tsimgs,(12000,784))
tsimgs1 = []
for v in tsimgs:
...
for itr in range(itrs):#iterations
    wjk = wkj.transpose()[1:]
    for i in range(48000):#loop on images
        x = trimgs[i]
        t = lbvecs[i]
